...
const stationeryInputSchema = z.object({
...
export async function GET(request: NextRequest) {
    ...
    ...(q
      ? {
          OR: [
            ...
            { itemType: { contains: q, mode: "insensitive" as const } },
          ],
        }
      : {}),
  };
  ...
  const [items, total] = await Promise.all([
    prisma.stationeryItem.findMany({ where, orderBy, skip: (page - 1) * pageSize, take: pageSize }),
...
export async function POST(request: NextRequest) {
  ...
  const parsed = stationeryInputSchema.safeParse(body);
  if (!parsed.success) {
    return NextResponse.json({ error: "Geçersiz veri.", details: parsed.error.flatten() }, { status: 400 });
  }

  const item = await prisma.stationeryItem.create({ data: parsed.data });
